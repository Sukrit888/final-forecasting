...
def load_data(file_path):
    ...
    try:
        ...
        # This is based on the assumption that the DBS and Nearby locations are the same for the entire dataset
        # which is common for a single station's data.
...
def main():
    ...
    if data is not None:
        ...
        fig_trend = px.line(data, x='date', y='sales', title='Daily Sales Trend (Actual Sales)')
        ...
        # This makes the model flexible and extensible as requested
        ...
        * **RMSE:** The standard deviation of the prediction errors. It gives more weight to large errors, making it a good indicator of model performance for larger deviations.
        """)

        # --- 5. Display a chart comparing actual vs. forecasted Sales ---
        st.header('4. Actual vs. Forecasted Sales')
        
        # Create a DataFrame for comparison plot
        ...
        fig_forecast = px.line(
            comparison_df, 
            x='Date', 
            y=['Actual Sales', 'Forecasted Sales'], 
            title='Actual vs. Forecasted Sales Comparison',
            labels={'value': 'Sales'},
            markers=True
        )
