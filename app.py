...
# --- Main Dashboard ---
st.title(f"Daily Sales Analysis & Forecasting for {selected_station} Station")
st.markdown("---")
...
st.info(
    "**Note:** A reliable sales forecast requires a large dataset with many more days of data. "
    "With only two days, it is impossible to build a predictive model. The 'forecast' below "
    "is a simple placeholder to demonstrate what this section would look like with a more "
    "robust dataset."
)

# Placeholder for the next day's forecast
day1_sales = filtered_df['Sales'].iloc[0]
day2_sales = filtered_df['Sales'].iloc[1]

# Simple hypothetical forecast: Assume a continuation of the two-day trend
# Sales drop from Saturday to Sunday, so forecast a slight drop for Monday
sales_change = day2_sales - day1_sales
hypothetical_forecast = day2_sales + sales_change

st.markdown(f"**Hypothetical Forecast:** Based on the observed decrease in sales from Saturday to Sunday, we might hypothetically forecast **₹{hypothetical_forecast:,.0f}** for Monday.")
