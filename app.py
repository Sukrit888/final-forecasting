...
def forecast_multiple_years(day_sales, day_reasoning, num_years):
    ...
    for year in range(1, num_years + 1):
        forecasted_sales, forecast_reasoning = forecast_sales(current_sales, day_reasoning)
        year_label = f"Year {2023 + year}"
        all_forecasts[year_label] = pd.DataFrame([forecasted_sales, forecast_reasoning], index=['Forecasted Average Sales (Units)', 'Forecast Reasoning']).T
...
if day_sales_2023:
    ...
    line_chart_data = {
        ...
        f"2023 Actual": list(day_sales_2023.values())
    }
