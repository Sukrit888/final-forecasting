...
def generate_note_without_api(input_reasoning):
    """Generates a predictive note based on a rule-based system."""
    if not input_reasoning:
        return "No significant change expected."

    positive_keywords = [k for k in IMPACT_FACTORS["positive"].keys() if k in input_reasoning.lower()]
    negative_keywords = [k for k in IMPACT_FACTORS["negative"].keys() if k in input_reasoning.lower()]

    if positive_keywords and negative_keywords:
        return "Mixed outlook due to conflicting positive and negative factors from the previous year. Monitor closely."
    elif positive_keywords:
        return f"Positive momentum is expected to continue based on historical trends. "
    elif negative_keywords:
        return f"Potential for a downturn due to historical negative factors. "
    else:
        return "Stable conditions anticipated with no major influencing factors identified."
...
def forecast_sales(day_sales, day_reasoning):
    ...
    for day, reason in day_reasoning.items():
        ...
        # Use the rule-based system to generate a new note
        note_text = generate_note_without_api(reason)
        notes[day] = note_text
