...
# Get unique stations for the selectbox, dropping any empty or NaN values
stations = df['DBS'].dropna().unique()
...
if not filtered_df.empty:
    ...
    # Check if there are at least two days of data for comparison
    if len(filtered_df) >= 2:
        ...
        st.warning("Only one day of data is available for this station. Cannot show a two-day comparison.")
        # Optional: Add a single-day visualization here if desired
        st.metric(label=f"Sales on {filtered_df['Day of the week'].iloc[0]}", value=f"{filtered_df['Sales'].iloc[0]:,}")
