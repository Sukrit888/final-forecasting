...
def load_data():
    """
    Loads the sales data from the CSV file and performs data cleaning.
    """
    try:
        df = pd.read_csv('GAIL Varanasi Sales Forecast.xlsx - Sheet1.csv')
        df['Date'] = pd.to_datetime(df['Date']).dt.date
        df['DBS'] = df['DBS'].ffill()
        return df
    except FileNotFoundError:
        st.error("Error: The file 'GAIL Varanasi Sales Forecast.xlsx - Sheet1.csv' was not found.")
        st.stop()
