...
def main():
    ...
    if data is not None:
        ...
        st.write("A table showing the sales trend over the available dates with influencing factors.")
        
        st.dataframe(data[['date', 'day_of_the_week', 'temperature_high_deg_c', 'temperatue_low_deg_c', 'nearby', 'sales']].set_index('date').rename(columns={'sales': 'Actual Sales', 'temperature_high_deg_c': 'Temp (High)', 'temperatue_low_deg_c': 'Temp (Low)'}), use_container_width=True)
        st.markdown("---")

        # --- 3. Build a forecasting model ---
        st.header('2. Sales Forecasting Model')
        st.write("The sales forecast is based on a Linear Regression model that uses the following factors:")
        st.markdown("""
        * **Day of the week:** The specific day (e.g., Monday, Saturday)
        * **Temperature:** Both the high and low temperatures for the day
        * **Nearby:** The location of nearby landmarks or establishments
        """)
